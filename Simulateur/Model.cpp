...
void Model::traverseNode(unsigned int nextNode, glm::mat4 matrix)
{
	...
	//decale le modele si une donnée de translation existe
	...
	//modifie la matrice par les données de translation rotation et echelle précédentes
...
std::vector<float> Model::getFloats(json accessor)
{
	...
	else throw std::invalid_argument("Type invalide type autorisé : ( SCALAR, VEC2, VEC3, or VEC4)");
...
std::vector<Texture> Model::getTextures()
{
	...
	for (unsigned int i = 0; i < JSON["images"].size(); i++)
	{
		...
		//test si la texture a déja été chargé  dans le cas ou une texture est utilisé plusieurs fois dans le model
		bool skip = false;
		...
		//si la texture a déja été chargée on passe 
		if (!skip)
		{
